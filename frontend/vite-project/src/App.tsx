...
const API_BASE_URL = import.meta.env.DEV
  ...
  const FRESH_TARGET = 4 * 10000000;
  const REPEAT_TARGET = 4 * 10000000;
 ...
 const fetchLeaderboard = useCallback(async () => {
  ...
  const applyDateRange = () => {
    ...
    const downloadFullReport = () => {
      ...
      reportData.push(["EazyPaisa - FULL REPORT"]);
      ...
      link.setAttribute('download', `EazyPaisa_Full_Report_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    const downloadIncentiveReport = () => {
      ...
      incentiveData.push(["EazyPaisa - INCENTIVE REPORT (80%+ RECEIVED)"]);
      ...
      let fileName = `EazyPaisa_Incentive_Report_${new Date().toISOString().split('T')[0]}`;
    ...
    //   name,
    //   email: `${name.toLowerCase().replace(/\s/g, '')}@EazyPaisaloan.com`
    // }));
